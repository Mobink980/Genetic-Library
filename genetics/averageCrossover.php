...
<ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Average Crossover (AX)</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="reduceSurrogateCrossover.php" class="btn"> Previous</a>
        <a id="btnNext" href="discreteCrossover.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Average Crossover (AX)</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In average crossover, the Gene of the Offspring in each position is calculated by the 
                    average of each of its parents. this is an example of how it works.
                </p>

                <p>This is an example for how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Crossover/average_crossover.png" alt="Average Crossover (AX)"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 26; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=26" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
"Average crossover" <br>
def average_crossover(parent_one, parent_two, pc): <br>
    """ <br>
    This function takes two parents, and performs Average crossover on them.  <br>
    ...
    <br>
    parent_one = Chromosome(genes= np.array([5, 3, 3, 2, 3, 9, 7, 6, 5, 4]),id_=0,fitness = 125.2)  # childs <br>
    parent_two = Chromosome(genes= np.array([5, 4, 7, 6, 5, 2, 6, 1, 3, 0]),id_=1,fitness = 125.2) <br>
    <br>
    print("\nParents") <br>
    print("=================================================")    <br> 
    ...
    if np.random.rand() < pc:  # if pc is greater than random number <br>
        for i in range(chrom_length): <br>
            child_one.genes[i] = int((parent_one.genes[i] + parent_two.genes[i])/2) <br>
            <br>
...
CROSS = average_crossover(parent1, parent2, 1) <br>
<br>
