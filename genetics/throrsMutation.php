...
<div id="leftSideBar" style="margin-top: 90px;">
            <ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Thrors Mutation</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="throasMutation.php" class="btn"> Previous</a>
        <a id="btnNext" href="distanceBasedMutation.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Thrors Mutation</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In thrors mutation, we select three positions that are 
                    not necessarily in a sequence and then we do the following.
                </p>
                <p>1.Placing the last element, in the first position in child-one.</p>
                <p>2.Placing the second element, in the last position in child-one.</p>
                <p>3.Placing the first element, in the second position in child-one.</p>
                <p>All the other elements are directly transferred from parent-one to child-one.</p>
                <p>This is an example for how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Mutation/thrors_mutation.png" alt="Thrors Mutation"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 67; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=67" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
def sorted_random_numbers(point_nums, interval): <br>
    ...
    <br>
"Thrors mutation" <br>
def thrors_mutation(parent_one, pm): <br>
    """ <br>
    This function takes one chromosome, and performs Thrors mutation on it.  <br>
    Last ==> First <br>
    Second ==> Last <br>
    First ==> Second  <br>
    In compare to Throas mutation, in Thrors mutation, the chosen elements are not necessarily <br> 
    successive.  <br>
    ...
    if np.random.rand() < pm:  # if pm is greater than random number <br>
        ...
        <br>
        for i in range(len(points)): #selecting the elements <br>
            elements.append(parent_one.genes[points[i]])  <br>
        # print(elements) <br>
        elements_copy = elements.copy() <br>
        elements[0] = elements_copy[-1]  <br>
        elements[-1] = elements_copy[1]  <br>
        elements[1] = elements_copy[0]  <br>
        # print(elements) <br>
        k = 0 <br>
        for i in range(chrom_length): <br>
            if i in points: <br>
                child_one.genes[i] = elements[k] <br>
                k+=1 <br>
...
MUTATION = thrors_mutation(parent1, 1) <br>
<br>
