...
<ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Modified Partially Mapped Crossover (MPMX)</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="oneBitAdaptiveCrossover.php" class="btn"> Previous</a>
        <a id="btnNext" href="orderBasedCrossover.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Modified Partially Mapped Crossover (MPMX)</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In modified partially mapped crossover, we first produce two randomly selected positions. 
                    We transfer every gene between these two points directly from parent one to child one and 
                    from parent two to child two. then we do the following:
                </p>
                <p>1. for child one, we fill the positions that can be filled by directly transferring the 
                genes in these positions from parent two to child one. then we shuffle the remaining elements
                and fill the empty positions in child one with them. </p>
                <p>2. for child two we fill the positions that can be filled by directly transferring the genes
                in those positions from parent one to child two. Then we shuffle the remaining elements and fill
                the empty positions in child two with them. </p>
...
def sorted_random_numbers(point_nums, interval):   <br>
    ...
    <br>
"Modified partially mapped crossover"   <br>
def modified_partially_mapped_crossover(parent_one, parent_two, pc):   <br>
    """   <br>
    This function takes two parents, and performs Modified partially mapped crossover on them.     <br>
    ...
    <br>
    parent_one = Chromosome(genes= np.array([0, 8, 4, 5, 6, 7, 1, 2, 3, 9]),id_=0,fitness = 125.2)  # parents   <br>
    parent_two = Chromosome(genes= np.array([6, 7, 1, 2, 4, 8, 3, 5, 9, 0]),id_=1,fitness = 125.2)   <br>
    <br>
    print("\nParents")   <br>
    print("=================================================")   <br>
    ...
    not_seen_list1 = [] #Elements that are not transfered to child_one   <br>
    not_seen_list2 = [] #Elements that are not transfered to child_two   <br>
    <br>
    if np.random.rand() < pc:  # if pc is greater than random number   <br>
        ...
        for i in range(chrom_length):   <br>
            if i < points[0] or i >= points[1]:   <br>
                if parent_two.genes[i] not in seen_list1:   <br>
                    child_one.genes[i] = parent_two.genes[i]   <br>
                    seen_list1.append(parent_two.genes[i])   <br>
                if parent_one.genes[i] not in seen_list2:   <br>
                    child_two.genes[i] = parent_one.genes[i]   <br>
                    seen_list2.append(parent_one.genes[i])   <br>
                    <br>
        for i in range(chrom_length):   <br>
            if parent_one.genes[i] not in seen_list1:   <br>
                not_seen_list1.append(parent_one.genes[i])   <br>
            if parent_one.genes[i] not in seen_list2:   <br>
                not_seen_list2.append(parent_one.genes[i])   <br>
                <br>
        np.random.shuffle(not_seen_list1) #shuffling the elements that are not present in childs    <br>
        np.random.shuffle(not_seen_list2)   <br>
        k = 0   <br>
        m = 0   <br>
        for i in range(chrom_length):   <br>
            if child_one.genes[i] == -1:   <br>
                child_one.genes[i] = not_seen_list1[k]   <br>
                k+=1   <br>
            if child_two.genes[i] == -1:   <br>
                child_two.genes[i] = not_seen_list2[m]   <br>
                m+=1                         <br>
...
CROSS = modified_partially_mapped_crossover(parent1, parent2, 1)   <br>
<br>
...
# Genes=   <br>
# [0 8 4 5 6 7 1 2 3 9]   <br>
# ID=#1, Fitness=125.2,   <br>
# Genes=   <br>
# [6 7 1 2 4 8 3 5 9 0]   <br>
<br>
# Children   <br>
# =================================================   <br>
# ID=#0, Fitness=125.2,   <br>
# Genes=   <br>
# [4 7 1 5 6 8 3 2 9 0]   <br>
# ID=#1, Fitness=125.2,   <br>
# Genes=   <br>
# [0 8 6 2 4 7 1 5 3 9]   <br>
  </div>  
</div>
