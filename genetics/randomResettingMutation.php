...
<div id="leftSideBar" style="margin-top: 90px;">
            <ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Random Resetting Mutation</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="bitFlipMutation.php" class="btn"> Previous</a>
        <a id="btnNext" href="swapMutation.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Random Resetting Mutation</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In random resetting mutation, one or several positions are randomly selected, 
                    and a value is determined for each of those positions, from the allowed or valid 
                    domain. (A domain could be the positive integers.)
                </p>
                <p>This is an example for how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Mutation/random_resetting_mutation.png" alt="Random Resetting Mutation"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 54; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=54" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
def sorted_random_numbers(point_nums, interval):  <br>
    ...
    <br>
    <br>
"Random resetting mutation"  <br>
def random_resetting_mutation(parent_one, pm):  <br>
    """  <br>
    This function takes one chromosome, and performs Random resetting mutation on it.   <br>
    ...
    if np.random.rand() < pm:  # if pm is greater than random number  <br>
        ...
        for i in range(len(points)):   <br>
            parent_one.genes[points[i]] = np.random.randint(1, 10) #one of the admissible values  <br>
...
MUTATION = random_resetting_mutation(parent1, 1)  <br>
<br>
