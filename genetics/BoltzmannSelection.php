...
<div id="leftSideBar" style="margin-top: 90px;">
        <ul>
            ...
            <li id="leftSideBarSelected"><a href="#">Boltzmann Selection</a></li>
        </ul>
    </div>
    ...
    <div class="topButtons">
        ...
        <h1>Boltzmann Selection</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    Boltzmann Selection, is based on the thermodynamical principles of simulated annealing.
                    By the passage of the time, the selection pressure will increase and the selection will be 
                    biased towards individuals with better finesses.
    ...
    $page_id = 15; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=15" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
def fitness_function(chrom): #TODO needs to be more generalize and useful <br>
    ...
    return fitness <br>
    <br>
def boltzmann_selection(selection_size, pop):  <br>
    """ <br>
    Boltzman Selection, is based on the thermodynamical principles of simulated annealing. <br>
    By the passage of the time, the selection pressure will increase and the selction will be biased twoards individuals with better fitnesses. <br>
    In this method, the function which enforces selection pressure is linear. (By the variable j) <br>
    ...
    select = []     <br>
    <br>
    for j in range(selection_size): #The probabilities of selections will change everytime before picking an individual <br>
        for i in range(pop_size): #calculating the probability of selection for each individual <br>
            probs[i] = round(abs(1/(1 + np.exp(-fitness_function(pop[i].genes)/(j + 1)))), 3) <br>
            <br>
        select.append(random.choices(pop, probs, k = 1))        <br>
        <br>
    flat_list = [item for sublist in select for item in sublist] <br>
    <br>
    return flat_list <br>
    <br>
    <br>
...
for i in range(len(POP)): #printing the population <br>
    ...
    <br>
SELECTION = boltzmann_selection(5, POP) <br>
<br>
