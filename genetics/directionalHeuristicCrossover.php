...
<ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Directional Heuristic Crossover</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="linearRecombinationCrossover.php" class="btn"> Previous</a>
        <a id="btnNext" href="geometricalCrossover.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Directional Heuristic Crossover</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In directional heuristic crossover, the offspring is produced as follow:
                </p>
                <p>For gene i of the child one to be filled, we have:</p>
                <p>child-one[i] = U(0, 1)*(parent-two[i] - parent-one[i]) + parent-two[i]</p>
                <p>child-one is the offspring.</p>
                <p>parent-one is the first parent that is selected from the population. 
                parent-two is the second parent that is selected from the population.</p>
                <p>This crossover is subject to the constraint that parent-two cannot be worse than parent-one. </p>
                <p>This is an example of how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Crossover/directional_heuristic_crossover.png" alt="Directional Heuristic Crossover"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 48; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=48" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
"Directional heuristic crossover"   <br>
def directional_heuristic_crossover(parent_one, parent_two, pc):   <br>
    """   <br>
    This function takes two parents, and performs Directional heuristic crossover on them.   <br>
    This crossover is subject to the constraint that the parent_two cannot be worth than parent_one.    <br>
    ...
    <br>
    parent_one = Chromosome(genes= np.array([0.121, 0.152, 0.231, 0.143, 0.732, 0.315, 0.434, 0.633]),id_=0,fitness = 129.3)  # childs   <br>
    parent_two = Chromosome(genes= np.array([0.765, 0.168, 0.914, 0.435, 0.893, 0.332, 0.981, 0.194]),id_=1,fitness = 127.2)   <br>
    <br>
    if parent_one.fitness > parent_two.fitness: #if parent_one is better, switch the parents   <br>
        parent_one, parent_two = parent_two, parent_one   <br>
        <br>
    print("\nParents")   <br>
    print("=================================================")   <br>
    ...
    child_two = Chromosome(genes= np.array([0.0]*chrom_length),id_=1,fitness = 125.2)   <br>
    <br>
    <br>
    if np.random.rand() < pc:  # if pc is greater than random number   <br>
        for i in range(chrom_length):   <br>
            child_one.genes[i] = abs(round(np.random.uniform(low=0.0, high=1.0)*(parent_two.genes[i] - parent_one.genes[i]) + parent_two.genes[i], 3))   <br>
            child_two.genes[i] = abs(round(np.random.uniform(low=0.0, high=1.0)*(parent_two.genes[i] - parent_one.genes[i]) + parent_two.genes[i], 3))    <br>
            <br>
            <br>
...
CROSS = directional_heuristic_crossover(parent1, parent2, 1)   <br>
<br>
...
# Genes=   <br>
# [0.115 0.145 0.189 0.122 0.606 0.299 0.412 1.034]   <br>
# ID=#1, Fitness=125.2,   <br>
# Genes=   <br>
# [0.11  0.136 0.131 0.034 0.68  0.308 0.12  1.063]   <br>
  </div>  
</div>
