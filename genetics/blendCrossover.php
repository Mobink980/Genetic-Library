...
<ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Blend Crossover (BLX)</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="heuristicUniformCrossover.php" class="btn"> Previous</a>
        <a id="btnNext" href="linearRecombinationCrossover.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Blend Crossover (BLX)</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In Blend crossover to perform the operation, we need three parameters. 
                    A list named d which will calculate the distance between the gene 
                    values of the parents in one position. A positive real parameter named
                    'a' in the interval [0, 1] which is selected randomly before the operation. 
                    Then for each Gene of the children, we create a random real uniform number u, 
                    for each offspring and value of the Gene in that position will become this 
                    random real uniform number. u is calculated in the following:
                </p>
                <p>u ~ (min(parent_one[i], parent_two[i]) - a* d[i], max(parent_one[i], parent_two[i]) + a* d[i])</p>
                <p>This is an example of how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Crossover/blend_crossover.png" alt="Blend Crossover (BLX)"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 46; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=46" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
class Chromosome():  <br>
    ...
    def get_chrom_length(self):   <br>
        ...
        <br>
"Blend crossover"  <br>
def blend_crossover(parent_one, parent_two, pc):  <br>
    """  <br>
    This function takes two parents, and performs Blend crossover on them.   <br>
    ...
    if np.random.rand() < pc:  # if pc is greater than random number  <br>
        ...
        for i in range(chrom_length):  <br>
            distance_vector.append(abs(parent_one.genes[i] - parent_two.genes[i]))  <br>
        # print(distance_vector)  <br>
        a = np.random.uniform(low = 0, high = 1)  <br>
        for i in range(chrom_length):  <br>
            w = np.random.uniform(low = abs(np.minimum(parent_one.genes[i], parent_two.genes[i]) - a * distance_vector[i]), high = abs(np.maximum(parent_one.genes[i], parent_two.genes[i]) + a * distance_vector[i]))  <br>
            child_one.genes[i] = round(w, 3)   <br>
            w = np.random.uniform(low = abs(np.minimum(parent_one.genes[i], parent_two.genes[i]) - a * distance_vector[i]), high = abs(np.maximum(parent_one.genes[i], parent_two.genes[i]) + a * distance_vector[i]))  <br>
            child_two.genes[i] = round(w, 3)  <br>
            <br>
...
CROSS = blend_crossover(parent1, parent2, 1)  <br>
<br>
...
# ID=#0, Fitness=125.2,  <br>
# Genes=  <br>
# [0.582 0.145 0.594 0.474 0.883 0.346 0.749 0.371]  <br>
# ID=#1, Fitness=125.2,  <br>
# Genes=  <br>
# [0.469 0.153 1.406 0.284 1.013 0.3   0.754 0.256]  <br>
  </div>  
</div>
