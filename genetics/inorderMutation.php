...
<div id="leftSideBar" style="margin-top: 90px;">
            <ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Inorder Mutation</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="inversionMutation.php" class="btn"> Previous</a>
        <a id="btnNext" href="centerinversionMutation.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Inorder Mutation</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In inorder mutation, several positions are selected, 
                    and for each position, a uniform random real number, w is produced.
                </p>
                <p>
                    If w &lt; 0.5 for that position, the value of the gene in that position 
                    is flipped. Otherwise, the value of the gene will remain the same.
                </p>
                <p>This is an example for how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Mutation/inorder_mutation.png" alt="Inorder Mutation"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 58; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=58" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
def sorted_random_numbers(point_nums, interval):  <br>
    ...
    <br>
"Inorder mutation"  <br>
def inorder_mutation(parent_one, pm):  <br>
    """  <br>
    This function takes one chromosome, and performs Inorder mutation on it.   <br>
    parent_one: The parent  <br>
    pm: The probability of mutation  <br>
    """   <br>
    parent_one = Chromosome(genes= np.array([0, 1, 0, 1, 1, 0, 1, 1, 0]),id_=0,fitness = 125.2) #parent  <br>
    print("\nParent")  <br>
    print("=================================================")  <br>
    ...
    if np.random.rand() < pm:  # if pm is greater than random number  <br>
        point_nums = np.random.randint(1, 5) #selecting one to four elements   <br>
        points = sorted_random_numbers(point_nums, chrom_length)  <br>
        child_one.genes = parent_one.genes  <br>
        print("points:", points)      <br>
        for i in range(point_nums):  <br>
            w = np.random.uniform(low=0, high=1)  <br>
            if w < 0.5:  <br>
                child_one.genes[points[i]] = child_one.genes[points[i]] ^ 1 #flipping the bit  <br>
                <br>
...
MUTATION = inorder_mutation(parent1, 1)  <br>
<br>
