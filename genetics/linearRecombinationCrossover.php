...
<ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Linear Recombination Crossover</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="blendCrossover.php" class="btn"> Previous</a>
        <a id="btnNext" href="directionalHeuristicCrossover.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Linear Recombination Crossover</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In linear recombination crossover, we select two parents and create three offspring.
                    If we name the first parent as parents one and second parent as a parent as parent two,
                    the three offspring are calculated as follow:
                </p>
                <p>First offspring: parent_one + parent_two</p>
                <p>Second offspring: (1.5)*parent_one - (0.5)*parent_two</p>
                <p>Third offspring: (-0.5)*parent_one + (1.5)*parent_two</p>
                <p>After the production of the Offspring, we choose the two fittest offspring and return them as a result. </p>
                <p>This is an example of how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Crossover/linear_recombination_crossover.png" alt="Linear Recombination Crossover"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 47; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=47" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
"Linear crossover"   <br>
def linear_recombination_crossover(parent_one, parent_two, pc): #TODO fitness based, search fitness based functions and do correction   <br>
    """   <br>
    This function takes two parents, and performs Linear crossover on them.    <br>
    ...
    child_one = Chromosome(genes= np.array([0.0]*chrom_length),id_=0,fitness = 130.3)  # childs   <br>
    child_two = Chromosome(genes= np.array([0.0]*chrom_length),id_=1,fitness = 129.2)   <br>
    child_three = Chromosome(genes= np.array([0.0]*chrom_length),id_=2,fitness = 129.3)   <br>
    <br>
    if np.random.rand() < pc:  # if pc is greater than random number   <br>
    <br>
        for i in range(chrom_length):   <br>
            child_one.genes[i] = abs(parent_one.genes[i] + parent_two.genes[i])   <br>
            child_two.genes[i] = abs((1.5)*parent_one.genes[i] - (0.5)*parent_two.genes[i])   <br>
            child_three.genes[i] = abs((-0.5)*parent_one.genes[i] + (1.5)*parent_two.genes[i])   <br>
            <br>
        # Chromosome.describe(child_one)   <br>
        # Chromosome.describe(child_two)     <br>
        # Chromosome.describe(child_three)    <br>
        childs = [] #List containing the childs   <br>
        childs.append(child_one)   <br>
        childs.append(child_two)   <br>
        childs.append(child_three)   <br>
        <br>
        fitnesses = [] #List containing the fitnesses   <br>
        fitnesses.append(child_one.fitness)   <br>
        fitnesses.append(child_two.fitness)   <br>
        fitnesses.append(child_three.fitness)   <br>
        <br>
        minimum_fitness_index = fitnesses.index(np.min(fitnesses))   <br>
        childs.pop(minimum_fitness_index) #removing the child with the worth fitness   <br>
        <br>
        # print("=================================================")       <br>
        # for i in range(2):   <br>
        #     Chromosome.describe(childs[i])   <br>
        <br>
        child_one = childs[0]   <br>
        child_two = childs[1]   <br>
        <br>
...
CROSS = linear_recombination_crossover(parent1, parent2, 1)   <br>
<br>
...
# Children   <br>
# =================================================   <br>
# ID=#0, Fitness=130.3,   <br>
# Genes=   <br>
# [0.886 0.32  1.145 0.578 1.625 0.647 1.415 0.827]   <br>
# ID=#2, Fitness=129.3,   <br>
# Genes=   <br>
# [1.087  0.176  1.2555 0.581  0.9735 0.3405 1.2545 0.0255]   <br>
  </div>  
</div>
