...
<ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Heuristic Crossover (HC)</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="cycleCrossover.php" class="btn"> Previous</a>
        <a id="btnNext" href="shuffleCrossover.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Heuristic Crossover(HC) or Intermediate Crossover(IC)</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In this crossover two parents are selected from the population and one 
                    offspring is produced. We first create a random number between 0 and 1. 
                    Then the genes of the child chromosome will be calculated as follow:
                </p>
                <p>child_one[i] = parent_one[i] + a*(parent_two[i] - parent_one[i])</p>
                <p>Variable a is a random number between 0 and 1.</p>
                <p>This is an example for how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Crossover/heuristic_crossover.png" alt="Heuristic Crossover(HC) or Intermediate Crossover(IC)"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 23; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=23" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
