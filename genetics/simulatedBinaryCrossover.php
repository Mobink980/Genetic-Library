...
<ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Simulated Binary Crossover (SBX)</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="geometricalCrossover.php" class="btn"> Previous</a>
        <a id="btnNext" href="hillclimbingCrossover.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Simulated Binary Crossover (SBX)</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In simulated binary crossover, two offspring are produced,
                    which the value of their genes are calculated as follow:
                </p>
                <p>child-one[i] = (0.5) * [(1 + y_i)* parent-one[i] + (1 - y_i)*parent-two[i]]</p>
                <p>child-two[i] = (0.5) * [(1 - y_i)* parent-one[i] + (1 + y_i)*parent-two[i]] (fix)</p>
                <p>This is an example of how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Crossover/simulated_binary_crossover.png" alt="Simulated Binary Crossover (SBX)"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 50; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=50" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
class Chromosome():  <br>
    ...
    def get_chrom_length(self):   <br>
        ...
        <br>
        <br>
"Simulated binary crossover"  <br>
def simulated_binary_crossover(parent_one, parent_two, pc):  <br>
    """  <br>
    This function takes two parents, and performs Simulated binary crossover on them.  <br>
    This crossover, generates offsprings symmetrically about the parents. It has no bias towards any of the parents and   <br>
    large values of distribution index will result in higher probability that offsprings will be created near the parents and vice versa.   <br>
    ...
    if np.random.rand() < pc:  # if pc is greater than random number        <br>
        distribution_index = np.random.randint(1, chrom_length) #distribution_index > 0  <br>
        factor = 0  <br>
        for i in range(chrom_length):  <br>
            random_number = np.random.uniform(low=0.0, high=1.0)  <br>
            if random_number <= 0.5:  <br>
                factor = (2 * random_number) ** (1 / (distribution_index + 1))  <br>
            else:  <br>
                factor = (1 / (2 * (1 - random_number))) ** (1 / (distribution_index + 1))  <br>
                <br>
            child_one.genes[i] = round(abs(0.5 * ((1 + factor) * parent_one.genes[i] + (1 - factor) * parent_two.genes[i])), 3)  <br>   
            child_two.genes[i] = round(abs(0.5 * ((1 - factor) * parent_one.genes[i] + (1 + factor) * parent_two.genes[i])), 3)    <br>
            <br>
...
CROSS = simulated_binary_crossover(parent1, parent2, 1)  <br>
<br>
...
# ID=#0, Fitness=125.2,  <br>
# Genes=  <br>
# [0.03  0.151 0.245 0.079 0.703 0.318 0.429 0.629]  <br>
# ID=#1, Fitness=125.2,  <br>
# Genes=  <br>
# [0.916 0.169 0.9   0.499 0.922 0.329 0.986 0.198]  <br>
  </div>  
</div>
