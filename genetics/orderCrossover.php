...
<ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Order Crossover (ox)</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="multiParentCrossover.php" class="btn"> Previous</a>
        <a id="btnNext" href="partiallyMappedCrossover.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Order Crossover (ox)</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In order crossover, two points are selected randomly in the parents. 
                    Then the genes between these two points from parents one is directly transferred to 
                    child one and from parent two to child two. then for the child one we start from the second point 
                    of the parent two and we transfer to child one what is not already transferred to child one. 
                    We do the same thing for the child two with parent one.
                </p>
                <p>This is an example for how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Crossover/order_cross.png" alt="Order Crossover (ox)"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 20; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=20" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
"Order crossover"  <br>
def order_crossover(parent_one, parent_two, pc):  <br>
    """  <br>
    This function takes two parents, and performs Order crossover on them.   <br>
    ...
    parent_two = Chromosome(genes= np.array([4, 5, 2, 1, 8, 0, 7, 6, 9, 3]),id_=1,fitness = 125.2)  <br>
    <br>
    print("\nParents")  <br>
    print("=================================================")  <br>
    ...
    point1 = np.random.randint(1, int(chrom_length/2))  <br>
    point2 = np.random.randint(int(chrom_length/2), chrom_length)  <br>
    # print(point1)  <br>
    # print(point2)  <br>
    <br>
    trans_elements1 = [] #elements transfered to child1 from parent1  <br>
    trans_elements2 = [] #elements transfered to child2 from parent2  <br>
    list1 = [] #elements to be transfered to child1 from parent2   <br>
    list2 = [] #elements to be transfered to child2 from parent1  <br>
    <br>
    if np.random.rand() < pc:  # if pc is greater than random number  <br>
        for i in range(chrom_length):  # transfer genes  <br>
            if i >= point1 and i < point2:  <br>
                child_one.genes[i] = parent_one.genes[i]  <br>
                child_two.genes[i] = parent_two.genes[i]  <br>
                trans_elements1.append(child_one.genes[i])  <br>
                trans_elements2.append(child_two.genes[i])  <br>
        for i in range(point2,chrom_length):  <br>
            list1.append(parent_two.genes[i]) #for c1 from p2  <br>
            list2.append(parent_one.genes[i]) #for c2 from p1  <br>
        for i in range(point2):  <br>
            list1.append(parent_two.genes[i])   <br>
            list2.append(parent_one.genes[i])   <br>
        delete1 = [] #index of the elements that should be deleted from list1  <br>
        delete2 = [] #index of the elements that should be deleted from list2  <br>
        for i in range(chrom_length):  <br>
            if list1[i] in trans_elements1: delete1.append(i)  <br>
            if list2[i] in trans_elements2: delete2.append(i)  <br>
        count = 0  <br>
        for i in range(len(delete1)): #deleting the redundant elements  <br>
            list1.pop(delete1[i]-count)  <br>
            list2.pop(delete2[i]-count)  <br>
            count +=1  <br>
        count = 0  <br>
        for i in range(point2,chrom_length):  <br>
            child_one.genes[i] = list1[count]  <br>
            child_two.genes[i] = list2[count]  <br>
            count +=1   <br>
            <br>
...
CROSS = order_crossover(parent1, parent2, 1)  <br>
<br>
...
# Genes=  <br>
# [4 5 2 1 8 7 6 9 3]  <br>
<br>
# Children  <br>
# =================================================  <br>
# ID=#0, Fitness=125.2,  <br>
# Genes=  <br>
# [5 2 1 4 8 7 6 9 3]  <br>
# ID=#1, Fitness=125.2,  <br>
# Genes=  <br>
