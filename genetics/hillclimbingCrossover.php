...
<ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Hillclimbing Crossover</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="simulatedBinaryCrossover.php" class="btn"> Previous</a>
        <a id="btnNext" href="arithmeticCrossover.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Hillclimbing Crossover</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    Lorem ipsum dolor sit amet consectetur adipisicing elit. 
                    Eaque deleniti facere harum expedita recusandae quaerat 
                    rerum. Iure numquam autem odit nostrum tempora soluta quis 
                    eum? Incidunt, omnis libero. Mollitia, reiciendis. Lorem ipsum 
                    dolor sit amet consectetur adipisicing elit. Suscipit recusandae,
                     laborum similique molestias saepe libero neque sapiente quisquam
                      tenetur natus magni ea. Veniam tempora quod, quis explicabo minima
                       assumenda perferendis.
                </p>
               
                <p>This is an example of how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Crossover/hillclimbing_crossover.png" alt="Hillclimbing Crossover"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 51; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=51" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
class Chromosome():  <br>
    ...
    def get_chrom_length(self):   <br>
        ...
        <br>
        <br>
def claculate_mask(chrom_size, px):  <br>
    """  <br>
    This method, performs the mask calculation for the Uniform crossover.  <br>
    chrom_size: The mask has the same size of the chromosome.  <br>
    px: A control parameter for the number of ones in the mask vector.  <br>
    """    <br>
    mask = [0]*chrom_size  <br>
    for i in range(chrom_size):  <br>
        prob = np.random.rand()  <br>
        if prob <= px:  <br>
            mask[i]=1  <br>
    return mask  <br>
    <br>  <br>

"Uniform crossover"  #TODO explaining all the operators in a document.  <br>
def uniform_crossover(parent_one, parent_two, pc):  <br>
    """  <br>
    This function takes two parents, and performs Uniform crossover on them.   <br>
    ...
    if np.random.rand() < pc:  # if pc is greater than random number  <br>
        mask = claculate_mask(chrom_length,np.random.uniform(low=0.2, high=0.85))  <br>
        print("\n Mask Vector")  <br>
        print("=================================================")  <br>
        print(mask)  <br>
    ...
    <br>
def fitness_function(chrom): #TODO needs to be more generalize and useful  <br>
    """  <br>
    This function, takes a chromosome and returns a value as its fitness.  <br>
    chrom: The chromoseme which its fitness is calculated and returned.  <br>
    """    <br>
    fitness = 0  <br>
    for i in range(len(chrom)):  <br>
        if chrom[i]==1:  <br>
            fitness +=1  <br>
    return fitness  <br>
    <br>  <br>

"Hillclimbing crossover"  <br>
def hillclimbing_crossover(parent_one, parent_two, pc):  <br>
    """  <br>
    This function takes two parents, and performs Hillclimbing crossover on them.   <br>
    In this method, we use uniform crossover to produce offsprings; and we do so until an offspring is produced which is better  <br>
    than one of the parents.  <br>
    parent_one: First parent  <br>
    parent_two: Second parent  <br>
    pc: The probability of crossover  <br>
    """    <br>
    ...
    <br>
    # parent_one = Chromosome(genes= np.array([0.121, 0.152, 0.231, 0.143, 0.732, 0.315, 0.434, 0.633]),id_=0,fitness = 125.2)  # childs  <br>
    # parent_two = Chromosome(genes= np.array([0.765, 0.168, 0.914, 0.435, 0.893, 0.332, 0.981, 0.194]),id_=1,fitness = 125.2)  <br>
    <br>
    parent_one = Chromosome(genes= np.array([0, 1, 0, 0, 1, 0, 0, 1, 1, 1]),id_=0,fitness = 5)  # childs  <br>
    parent_two = Chromosome(genes= np.array([1, 0, 1, 0, 0, 1, 0, 1, 0, 0]),id_=1,fitness = 4) #repair fitness_function  <br>
    <br>
    print("\nParents")  <br>
    print("=================================================")  <br>
    Chromosome.describe(parent_one)  <br>
    ...
    <br>
    if np.random.rand() < pc:  # if pc is greater than random number  <br>
        maximum_parent_fitness = np.maximum(parent_one.fitness, parent_two.fitness)  <br>
        child_one, child_two = uniform_crossover(parent_one, parent_two, 1)  <br>
        child_one.fitness = fitness_function(child_one.genes)  <br>
        child_two.fitness = fitness_function(child_two.genes)  <br>
        maximum_child_fitness = np.maximum(child_one.fitness, child_two.fitness)  <br>
        <br>
        # print(maximum_child_fitness)  <br>
        if maximum_child_fitness <= maximum_parent_fitness:  <br>
            while maximum_child_fitness <= maximum_parent_fitness: #loop until one of the childs becomes better than parents  <br>
                child_one, child_two = uniform_crossover(parent_one, parent_two, 1)  <br>
                child_one.fitness = fitness_function(child_one.genes)  <br>
                child_two.fitness = fitness_function(child_two.genes)  <br>
                maximum_child_fitness = np.maximum(child_one.fitness, child_two.fitness)  <br>
...
CROSS = hillclimbing_crossover(parent1, parent2, 1)  <br>
<br>
