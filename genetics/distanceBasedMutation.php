...
<div id="leftSideBar" style="margin-top: 90px;">
            <ul>
                ...
                <li><a href="inversionMutation.php">Inversion Mutation </a></li>
                ...
                <li id="leftSideBarSelected"><a href="#">Distance Based Mutation (DMO)</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="throrsMutation.php" class="btn"> Previous</a>
        <a id="btnNext" href="displacementMutation.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Distance Based Mutation (DMO)</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In distance-based mutation, we select a random position and find the 
                    elements in a certain distance around the selected position.
                </p>
                <p>After finding the selected elements, we shuffle the whole interval.</p>
                <p>To calculate the selected elements, we define two variables.</p>
                <p>The variable g which indicates the randomly selected position, and 
                the variable distance to indicate which elements in either side of the 
                randomly selected element will be selected.</p>
                
                <p>This is an example for how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Mutation/distance_based_mutation.png" alt="Distance Based Mutation (DMO)"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 61; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=61" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
def sorted_random_numbers(point_nums, interval): <br>
    ...
    <br>
    <br>
"Distance based mutation" <br>
def distance_based_mutation(parent_one, pm): <br>
    """ <br>
    This function takes one chromosome, and performs Distance based mutation on it. <br>
    In this method, g is the position for the chosen gene. d is the distance from that position.  <br>
    ...
    if np.random.rand() < pm:  # if pm is greater than random number <br>
        interval = [] <br>
        d = np.random.randint(1, 4) <br>
        g = np.random.randint(d,chrom_length-d) <br>
        child_one.genes = parent_one.genes <br>
        for i in range(g - d, g + d + 1): #element in position g and d elements in each direction <br>
            interval.append(parent_one.genes[i])  <br>
        # print(interval) <br>
        np.random.shuffle(interval) <br>
        k = 0 <br>
        for i in range(g - d, g + d + 1): #Replacing elements <br>
            child_one.genes[i] = interval[k] <br>
            k+=1 <br>
            <br>
            <br>
...
MUTATION = distance_based_mutation(parent1, 1) <br>
<br>
