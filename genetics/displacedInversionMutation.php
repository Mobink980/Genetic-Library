...
<div id="leftSideBar" style="margin-top: 90px;">
            <ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Displaced Inversion Mutation</a></li>             
            </ul>
    </div>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="insertionMutation.php" class="btn"> Previous</a>
        <a id="btnNext" href="#" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Displaced Inversion Mutation</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In displaced inversion mutation, we select a random substring 
                    from the chromosome, then we reverse that substring and paste it
                    in another position, without changing the sequence of other elements.
                </p>
                <p>This is an example for how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Mutation/displaced_inversion_mutation.png" alt="Displaced Inversion Mutation"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 64; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=64" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
def sorted_random_numbers(point_nums, interval):   <br>
    ...
    <br>
    <br>
"Displaced inversion mutation"   <br>
def displaced_inversion_mutation(parent_one, pm):   <br>
    """   <br>
    This function takes one chromosome, and performs Displaced inversion mutation on it.    <br>
    ...
    if np.random.rand() < pm:  # if pm is greater than random number   <br>
        ...
        for i in range(chrom_length):   <br>
            if parent_one.genes[i] in interval:   <br>
                duplicate.remove(parent_one.genes[i])   <br>
                <br>
        # print(duplicate)   <br>
        lists = []   <br>
        ...
        # print(position)   <br>
        interval.reverse()   <br>
        for i in range(len(interval)):   <br>
            lists[position].append(interval[i])   <br>
            <br>
        k = 0   <br>
...
MUTATION = displaced_inversion_mutation(parent1, 1)   <br>
<br>
