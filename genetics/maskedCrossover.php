...
<ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Masked Crossover</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="randomRespectfulCrossover.php" class="btn"> Previous</a>
        <a id="btnNext" href="oneBitAdaptiveCrossover.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Masked Crossover</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In Masked crossover, we first copy the value of the genes from parent one 
                    to child one and from parent two to child two. In this crossover, we assume 
                    that each individual in the population has a mask vector. the value of the
                    cells of this vector is calculated based on the individual's fitness. if the
                    value of a mask vector in a position is 1 and the value of another mask 
                    vector in that position is 0, we say that the first mask vector is dominant
                    in that position. If we consider the mask_vector1 as the parent one, and 
                    the mask_vector2  as the mask vector of parent two, the offspring will be
                    calculated as the following.
                </p>
                <p>1. If mask_vector2[i] = 1 and mask_vector1[i] = 0, then child_one[i] = parent_two[i]</p>
                <p>2. If mask_vector2[i] = 0 and mask_vector1[i] = 1, then child_two[i] = parent_one[i]</p>
                <p>This is an example for how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Crossover/masked_crossover.png" alt="Masked Crossover"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 33; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=33" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
"Masked crossover"  <br>
def masked_crossover(parent_one, parent_two, pc):  <br>
    """  <br>
    This function takes two parents, and performs Masked crossover on them.  <br>
    In this crossover, each of the parents have a masked vector, and this vector for an individual   <br>
    is calculated based on the fitness of that individual within the population.  <br>
    In this method, we used vectors with random values for this purpose.      <br>
    ...
    parent_two = Chromosome(genes= np.array([1, 0, 1, 1, 1, 0, 1, 1, 0, 0]),id_=1,fitness = 125.2)  <br>
    <br>
    print("\nParents")  <br>
    print("=================================================")  <br>
    ...
    mask_one = np.random.choice([0, 1], size=chrom_length, p=[0.5, 0.5]) #mask vector for parent_one  <br>
    mask_two = np.random.choice([0, 1], size=chrom_length, p=[0.5, 0.5]) #mask vector for parent_two  <br>
    print("mask vector for parent_one:", mask_one)  <br>
    print("mask vector for parent_two", mask_two)  <br>
    <br>
    if np.random.rand() < pc:  # if pc is greater than random number  <br>
        for i in range(chrom_length): #Copying parents into children   <br>
            child_one.genes[i] = parent_one.genes[i]  <br>
            child_two.genes[i] = parent_two.genes[i]  <br>
            <br>
        for i in range(chrom_length):  <br>
            if mask_one[i] == 0 and mask_two[i] == 1: #The parent_two is dominant  <br>
                child_one.genes[i] = parent_two.genes[i]  <br>
            if mask_one[i] == 1 and mask_two[i] == 0: #The parent_one is dominant  <br>
...
CROSS = masked_crossover(parent1, parent2, 1)  <br>
<br>
