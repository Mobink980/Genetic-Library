...
<div id="leftSideBar" style="margin-top: 90px;">
            <ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Scramble Mutation </a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="swapMutation.php" class="btn"> Previous</a>
        <a id="btnNext" href="inversionMutation.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Scramble Mutation or Partial Shuffle Mutation (PSM)</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In scramble mutation, a substring of the chromosome 
                    is selected and the value of its genes are shuffled.
                </p>
                <p>This is an example for how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Mutation/scramble_mutation.png" alt="Scramble Mutation or Partial Shuffle Mutation (PSM)"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 56; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=56" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
def scramble_mutation(parent_one, pm):  <br>
    """  <br>
    This function takes one chromosome, and performs Scramble mutation on it.   <br>
    ...
    if np.random.rand() < pm:  # if pm is greater than random number  <br>
        ...
        for i in range(points[0], points[1]):  <br>
            permute.append(parent_one.genes[i])  <br>
        np.random.shuffle(permute)  <br>
        k = 0  <br>
        for i in range(points[0], points[1]):  <br>
            child_one.genes[i] = permute[k]  <br>
            k+=1        <br>
            <br>
...
MUTATION = scramble_mutation(parent1, 1)  <br>
<br>
