...
<div id="leftSideBar" style="margin-top: 90px;">
        <ul>
            ...
            <li id="leftSideBarSelected"><a href="#">Tournament  Selection</a></li>
    ...
    <div class="topButtons">
        ...
        <h1>Tournament  Selection</h1>
                <hr />
                <p>
                    In Tournament Selection, we select a group of individuals randomly (n_ts), 
                    and we choose the fittest individuals among them.
                </p>
  ...
  <?php
    ...
    $page_id = 6; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=6" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
def pop_sort(pop): <br>
    ...
    <br>
    <br>
def tournament_selection(selection_size, pop): <br>
    """ <br>
    In Tournament Selection, we select a group of individuals randomly, and we choose  <br>
    the fittest individuals among them. <br>
    This function takes a population, selects a group of chromosomes within, <br>
    and returns the fittest ones inside. <br>
    ...
    group_size = np.random.randint(selection_size,pop_size)#number of individuals to be picked <br>
    group = [] <br>
    for _ in range(group_size): <br>
        group.append(pop[np.random.randint(0,pop_size)]) <br>
    sorted_group = pop_sort(group) #sorting the group based on fitness <br>
    select = [] <br>
    for i in range(selection_size-1,-1,-1): <br>
        select.append(sorted_group[i]) <br>
    return select <br>
...
for i in range(len(POP)): #printing the population <br>
    ...
    <br>
SELECTION = tournament_selection(5, POP) <br>
<br>
