...
<ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Random Respectful Crossover (RRC)</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="modifiedOrderCrossover.php" class="btn"> Previous</a>
        <a id="btnNext" href="maskedCrossover.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Random Respectful Crossover (RRC)</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In random respectful crossover, we use a list named similarity vector 
                    to produce offspring. to calculate similarity vector, we compare the 
                    genes of the two parents in each position. if the value of the genes 
                    in that position is the same, the value of the similarity vector for 
                    that position becomes the value of the genes in that position. 
                    Otherwise, the value of the similarity vector for that position 
                    becomes -1. To calculate the values of the genes for the offspring we do the following.
                </p>
                <ul>
                    <li>if the value of the similarity Vector in position i is 0, the value of the 
                    genes in that position for both offspring becomes 0. </li>
                    <li>if the value of the similarity vector in positions i is 1, the value of the
                    genes in that position for both offspring becomes 1. </li>
                    <li>if the value of the similarity vector in position i is -1, 
                    we produce a uniform random real number w between 0 and 1 to determine 
                    what the value of office ring in that position would be. </li>
                    <ul>
                        <li>if w &lt 0.5, the value of the genes in that position for that offspring becomes 1.</li>
                        <li>otherwise, the value of the genes in that position for that offspring becomes 0. </li>
                    </ul>
                </ul>
                <p>This is an example for how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Crossover/random_respectful_crossover.png" alt="Random Respectful Crossover (RRC)"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 32; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=32" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
"Random respectful crossover"  <br>
def random_respectful_crossover(parent_one, parent_two, pc):  <br>
    """  <br>
    This function takes two parents, and performs Random respectful crossover on them.   <br>
    ...
    <br>
    parent_one = Chromosome(genes= np.array([1, 1, 1, 0, 1, 1, 0, 1, 0, 1]),id_=0,fitness = 125.2)  # parents  <br>
    parent_two = Chromosome(genes= np.array([1, 0, 1, 1, 1, 0, 1, 1, 0, 1]),id_=1,fitness = 125.2)  <br>
    <br>
    print("\nParents")  <br>
    print("=================================================")  <br>
    ...
    if np.random.rand() < pc:  # if pc is greater than random number  <br>
        similarity_vector = [-1]*chrom_length  <br>
        for i in range(chrom_length):  <br>
            if parent_one.genes[i] == parent_two.genes[i]:  <br>
                if parent_one.genes[i] == 0:  <br>
                    similarity_vector[i] = 0  <br>
                else:  <br>
                    similarity_vector[i] = 1  <br>
                    <br>
        print(similarity_vector)  <br>
        for i in range(chrom_length):  <br>
            if similarity_vector[i] == 1:  <br>
                child_one.genes[i] = 1  <br>
                child_two.genes[i] = 1  <br>
            elif similarity_vector[i] == 0:  <br>
                child_one.genes[i] = 0  <br>
                child_two.genes[i] = 0  <br>
            else:  #the similarity_vector[i] is -1  <br>
                w = np.random.uniform(low=0, high=1) #for the first child  <br>
                if w < 0.5:  <br>
                    child_one.genes[i] = 1  <br>
                else:  <br>
                    child_one.genes[i] = 0  <br>
                    <br>
                w = np.random.uniform(low=0, high=1) #for the second child  <br>
                if w < 0.5:  <br>
                    child_two.genes[i] = 1  <br>
                else:  <br>
                    child_two.genes[i] = 0  <br>
...
CROSS = random_respectful_crossover(parent1, parent2, 1)  <br>
<br>
