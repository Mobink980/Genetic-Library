...
<ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Modified Order Crossover (MOC)</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="countPreservingCrossover.php" class="btn"> Previous</a>
        <a id="btnNext" href="randomRespectfulCrossover.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Modified Order Crossover (MOC)</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In modified order crossover we choose a random position and then we 
                    find the left side of parent two in Parent one and transfer these genes 
                    from parent one to child one. then we loop over the right side of the 
                    parent two and fill the empty positions of the child one from left to 
                    right in the same order we do the same thing for the child two by finding 
                    the elements of the left side of parent one in parent two and transferring 
                    them and then filling the empty spots by the elements at the right side of the parent one.
                </p>

                <p>This is an example for how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Crossover/modified_order_crossover.png" alt="Modified Order Crossover (MOC)"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 31; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=31" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
"Modified order crossover"  <br>
def modified_order_crossover(parent_one, parent_two, pc):  <br>
    """  <br>
    This function takes two parents, and performs Modified order crossover on them.   <br>
    ...
    <br>
    parent_one = Chromosome(genes= np.array([0, 1, 2, 3, 4, 6, 9, 8, 5, 7]),id_=0,fitness = 125.2)  # childs  <br>
    parent_two = Chromosome(genes= np.array([2, 1, 9, 8, 0, 5, 6, 3, 7, 4]),id_=1,fitness = 125.2)  <br>
    <br>
    print("\nParents")  <br>
    print("=================================================")     <br> 
    ...
    if np.random.rand() < pc:  # if pc is greater than random number  <br>
        hold1 = [] #for holding the elements in the one half of the parent_two  <br>
        hold2 = [] #for holding the elements in the one half of the parent_one  <br>
        # pos = np.random.randint(1, chrom_length)  <br>
        pos = 4  <br>
        for i in range(pos):  <br>
            hold1.append(parent_two.genes[i]) #first half of the parent_two  <br>
            hold2.append(parent_one.genes[i]) #first half of the parent_one  <br>
        for i in range(chrom_length):  <br>
            if parent_one.genes[i] in hold1:  <br>
                child_one.genes[i] = parent_one.genes[i]  <br>
            if parent_two.genes[i] in hold2:  <br>
                child_two.genes[i] = parent_two.genes[i]  <br>
                <br>
        hold1 = []  <br>
        hold2 = []  <br>
        for i in range(pos,chrom_length):  <br>
            hold1.append(parent_two.genes[i]) #second half of the parent_two  <br>
            hold2.append(parent_one.genes[i]) #second half of the parent_one  <br>
            <br>
        k = 0  <br>
        m = 0  <br>
        for i in range(chrom_length):  <br>
            if child_one.genes[i] == -1:  <br>
                child_one.genes[i] = hold1[k]  <br>
                k+=1  <br>
            if child_two.genes[i] == -1:  <br>
                child_two.genes[i] = hold2[m]  <br>
                m+=1  <br>
                <br>
...
CROSS = modified_order_crossover(parent1, parent2, 1)  <br>
<br>
...
# Genes=  <br>
# [1 2 3 4 6 9 8 5 7]  <br>
# ID=#1, Fitness=125.2,  <br>
# Genes=  <br>
# [2 1 9 8 5 6 3 7 4]  <br>
<br>
# Children  <br>
# =================================================  <br>
# ID=#0, Fitness=125.2,  <br>
# Genes=  <br>
# [1 2 5 6 3 9 8 7 4]  <br>
# ID=#1, Fitness=125.2,  <br>
# Genes=  <br>
# [2 1 6 9 8 5 3 7 4]  <br>
  </div>  
</div>
