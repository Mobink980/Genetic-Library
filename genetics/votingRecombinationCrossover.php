...
<ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Voting Recombination Crossover (VR)</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="positionBasedCrossover.php" class="btn"> Previous</a>
        <a id="btnNext" href="maximalPreservationCrossover.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Voting Recombination Crossover (VR)</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In voting recombination crossover, we have several parents and a positive integer
                    as a threshold which is less than or equal to the number of parents. 
                    If the value of a gene in a position have been the same in a number of 
                    parents equal to or greater than the threshold, then the value of the gene 
                    in that position for the Offspring will become the same as that value. 
                    The remaining elements are shuffled and transferred to the offspring.                    
                </p>

                <p>In the following example, we have four parents and the threshold is 3.</p>

<img class="image-view" src="Genetics/Genetic pictures/Crossover/voting_recombination_crossover.png" alt="Voting Recombination Crossover (VR)"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 38; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=38" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
class Chromosome():   <br>
    ...
    def get_chrom_length(self):    <br>
        ...
        <br>   <br>
def most_frequent(List):   <br>
    """   <br>
    Returns the element with most occurrence in a list   <br>
    """    <br>
    occurence_count = Counter(List)    <br>
    return occurence_count.most_common(1)[0][0]    <br>
    <br>
"Voting recombination crossover" #TODO the arguments of this method, needs fixing.   <br>
def voting_recombination_crossover(parent_one, parent_two, pc):   <br>
    """   <br>
    This function takes several parents, and performs Voting recombination crossover on them.    <br>
    In this method, threshold is the number of repeatition for an element in one position, for    <br>
    the child would be the same element, in that position.    <br>
    parent_one: First parent   <br>
    parent_two: Second parent   <br>
    pc: The probability of crossover   <br>
    """     <br>
    parent_one =   Chromosome(genes= np.array([1, 4, 3, 5, 2, 6, 0]),id_=0,fitness = 125.2)  # parents   <br>
    parent_two =   Chromosome(genes= np.array([1, 2, 0, 3, 5, 6, 4]),id_=1,fitness = 125.2)   <br>
    parent_three = Chromosome(genes= np.array([3, 2, 1, 5, 4, 0, 6]),id_=2,fitness = 125.2)     <br>
    parent_four =  Chromosome(genes= np.array([1, 2, 3, 4, 0, 6, 5]),id_=3,fitness = 125.2)   <br>
    parent_five =  Chromosome(genes= np.array([0, 4, 2, 1, 6, 5, 3]),id_=4,fitness = 125.2)     <br>
    <br>
    parents = []   <br>
    ...
    threshold = 3   <br>
    seen_list = [] #Elements that are transfered to child   <br>
    not_seen_list = [] #Elements that are not transfered to child   <br>
    <br>
    if np.random.rand() < pc:  # if pc is greater than random number   <br>
        for i in range(chrom_length):   <br>
            column = []   <br>
            for j in range(len(parents)):   <br>
                column.append(parents[j].genes[i])   <br>
                <br>
            vote = most_frequent(column)   <br>
            votes_num = column.count(vote)   <br>
            if votes_num >= threshold:   <br>
                child_one.genes[i] = vote   <br>
                child_two.genes[i] = vote   <br>
                seen_list.append(vote)   <br>
                <br>
        for i in range(chrom_length):   <br>
            if parents[0].genes[i] not in seen_list:   <br>
                not_seen_list.append(parents[0].genes[i])   <br>
                <br>
        np.random.shuffle(not_seen_list)        <br>
        k = 0   <br>
        for i in range(chrom_length): #filling empty elements of first child   <br>
            if child_one.genes[i] == -1:   <br>
                child_one.genes[i] = not_seen_list[k]   <br>
                k+=1   <br>
                <br>
        np.random.shuffle(not_seen_list)        <br>
        k = 0   <br>
        for i in range(chrom_length): #filling empty elements of second child   <br>
            if child_two.genes[i] == -1:   <br>
                child_two.genes[i] = not_seen_list[k]   <br>
                k+=1   <br>
                <br>
    else:  # if pc is less than random number then the children will be one of the parents involved   <br>
        child_one = deepcopy(parents[np.random.randint(0, len(parents))])   <br>
        child_two = deepcopy(parents[np.random.randint(0, len(parents))])   <br>
...
CROSS = voting_recombination_crossover(parent1, parent2, 1)   <br>
<br>
