...
<ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Flat Crossover (FC)</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="discreteCrossover.php" class="btn"> Previous</a>
        <a id="btnNext" href="multivariateCrossover.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Flat Crossover (FC)</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In flat crossover for each gene of the child, we produce a uniform random real number, 
                    w between the values of the parents for that gene and we transfer this number to the child. 
                    In other words, if the parent one is the first parent and the parent two is the second parent 
                    and child one is the Offspring, then the value for the gene i of the child one is calculated as follows:
                </p>
                <p>child_one[i] ~ (minimum(parent_one[i], parent_two[i]), maximum(parent_one[i], parent_two[i]))</p>
                <p>This is an example for how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Crossover/flat_crossover.png" alt="Flat Crossover (FC)"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 28; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=28" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
"Flat crossover"  <br>
def flat_crossover(parent_one, parent_two, pc):  <br>
    """  <br>
    This function takes two parents, and performs Flat crossover on them.   <br>
    ...
    # c2 = chromosome(genes= np.array([-1]*chrom_length),id=1,fitness = 125.2)  <br>
    <br>
    if np.random.rand() < pc:  # if pc is greater than random number  <br>
        for i in range(chrom_length):  <br>
            w = np.random.uniform(low= np.minimum(parent_one.genes[i],parent_two.genes[i]), high= np.maximum(parent_one.genes[i],parent_two.genes[i]))  <br>
...
CROSS = flat_crossover(parent1, parent2, 1)  <br>
<br>
...
# ID=#0, Fitness=125.2,  <br>
# Genes=  <br>
# [0.311 0.159 0.522 0.254 0.76  0.317 0.909 0.234]  <br>
