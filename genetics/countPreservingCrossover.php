...
<ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Count Preserving Crossover (CPC)</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="multivariateCrossover.php" class="btn"> Previous</a>
        <a id="btnNext" href="modifiedOrderCrossover.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Count Preserving Crossover (CPC)</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In count preserving crossover the number of ones in the offspring 
                    should be equal to the number of ones in parents. Obviously, we assume 
                    that the number of ones in all individuals of the population is the same.
                </p>

                <p>This is an example for how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Crossover/count_preserving_cross.png" alt="Count Preserving Crossover (CPC)"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 30; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=30" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
"Count-preserving crossover"  <br>
def count_preserving_crossover(parent_one, parent_two, pc):  <br>
    """  <br>
    This function takes two parents, and performs Count-preserving crossover on them.  <br>
    In this crossover, the number of ones must be perserved in offsprings. We assume that   <br>
    the number of ones are equal in both parents.   <br>
    ...
    #number of 1s are equal in both parents  <br>
    parent_one = Chromosome(genes= np.array([1, 1, 1, 0, 1, 1, 0, 1, 0]),id_=0,fitness = 125.2)  # parents  <br>
    parent_two = Chromosome(genes= np.array([1, 0, 1, 1, 1, 0, 1, 1, 0]),id_=1,fitness = 125.2)  <br>
    <br>
    print("\nParents")  <br>
    print("=================================================")    <br>  
    ...
    up = [] #contains the positions of bits in the list which parent_one is one and parent_two is zero  <br>
    down = [] #contains the positions of bits in the list which parent_one is zero and parent_two is one  <br>
    <br>
    if np.random.rand() < pc:  # if pc is greater than random number  <br>
    <br>
        for i in range(chrom_length): #copying parents to children  <br>
        ...
        for i in range(chrom_length):  <br>
            if parent_one.genes[i] != parent_two.genes[i]:  <br>
                if parent_one.genes[i] == 1 and parent_two.genes[i] == 0:  <br>
                    up.append(i)  <br>
                else:  <br>
                    down.append(i)  <br>
        np.random.shuffle(up) #shuffle the lists to avoid producing similar childs   <br>
        np.random.shuffle(down)  <br>
        print(up)  <br>
        print(down)  <br>
        for i in range(len(up)): #changing the elements in the positions  <br>
            child_one.genes[up[i]], child_one.genes[down[i]] = child_one.genes[down[i]], child_one.genes[up[i]]   <br>
            child_two.genes[up[i]], child_two.genes[down[i]] = child_two.genes[down[i]], child_two.genes[up[i]]   <br>
            <br>  <br>  <br>
...
CROSS = count_preserving_crossover(parent1, parent2, 1)  <br>
<br>
