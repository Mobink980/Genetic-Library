...
<div id="leftSideBar" style="margin-top: 90px;">
        <ul>
            ...
            <li><a href="randomSelection.php">Random Selection</a></li>
            <li><a href="truncationSelection.php">Truncation Selection</a></li>
            <li><a href="tournamentSelection.php">Tournament  Selection</a></li>
            <li id="leftSideBarSelected"><a href="#">Stochastic Universal Sampling  Selection</a></li>
    ...
    <div class="topButtons">
        ...
        <h1>Stochastic Universal Sampling  Selection</h1>
                <hr />
                <p>
                    In Stochastic universal sampling Selection, Each individual has the probability of 
                    fitness(x(i))/total_fitnesses, to be selected. (the selection is biased towards individuals with better fitnesses)
                    The main difference between this selection and Roulette wheel selection is 
                    that it gives more opportunity to individual with lower fitnesses.
    ...
    $page_id = 7; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=7" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
def pop_sort(pop): <br>
    """ <br>
    This function sorts the population based on their fitnesses, using selection sort. <br>
    pop: The population that are sorted based on their fitnesses. <br>
    """   <br>
    for i in range(len(pop)):  <br>
        min_index = i  <br>
        for j in range(i+1, len(pop)):  <br>
            if pop[min_index].fitness > pop[j].fitness:  <br>
                min_index = j        <br>
        pop[i], pop[min_index] = pop[min_index], pop[i]    <br> 
    return pop <br>

    <br>
def stockasting_universal_sampling_selection(selection_size, pop):  <br>
    """ <br>
    In Stochastic universal sampling Selection, Each individual has the probability of fitness(x(i))/total_fitnesses,  <br>
    to be selected. (the selection is biased towards individuals with better fitnesses) <br>
    The main difference between this selection and Roulette wheel selection is that it gives more opportunity to individual with lower fitnesses. <br>
    This method, takes the population and returns several individuals among them. <br>
    ...
    total_fitnesses = 0.0 <br>
    probs = [0.0]*pop_size <br>
    select = [] <br>
    wheel = [] <br>
    sums = 0 <br>
    for i in range(pop_size): #calculating the fitnesses of the whole population <br>
        total_fitnesses += fitness_function(pop[i].genes) <br>
        <br>
    for i in range(pop_size): #calculating the probability of selection for each individual and also the Roulette wheel <br>
        probs[i] = abs(fitness_function(pop[i].genes)/total_fitnesses) <br>
        sums += probs[i] <br>
        wheel.append(sums) <br>

    # print(wheel) <br>
    random_numbers = [] #contains the hands on the wheel <br>
    bias = 1.0/selection_size #the distance between the hands  <br>
    random_number = np.random.uniform(low= 0.0, high = bias) <br>
    random_numbers.append(random_number) <br>
    for i in range(selection_size - 1):  <br>
        random_number += bias <br>
        random_numbers.append(random_number) <br>
        <br>
    for i in range(selection_size): <br>
        chosen = 0 #the index of the chosen chromosome <br>
        for j in range(len(wheel)): <br>
            if random_numbers[i] <= wheel[j]: <br>
                chosen = j <br>
                break <br>
                <br>
        select.append(pop[chosen]) <br>
    # print(random_numbers) <br>
    <br>
    return select <br>
    <br>
    <br>
...
for i in range(len(POP)): #printing the population <br>
    ...
    <br>
SELECTION = stockasting_universal_sampling_selection(5, POP) <br>
<br>
