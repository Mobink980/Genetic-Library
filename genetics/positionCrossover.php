...
<ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Position Crossover (PX)</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="maximalPreservationCrossover.php" class="btn"> Previous</a>
        <a id="btnNext" href="homologousCrossover.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Position Crossover (PX)</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In position crossover, several positions are randomly selected in parent 
                    one and transferred to child one. the transferred elements are deleted from parents
                    two and by a loop on remaining elements of the parents two the elements that
                    are not already transferred to child one are transferred to it.
                </p>
                <p>
                    For child two, in the same way, several positions are randomly selected in 
                    Parent two and transferred to child two. the transferred elements are deleted 
                    from parent one and by a loop on remaining elements of the parents one the 
                    elements that are not already transferred to child two are transferred to it. 
                    The main difference between position crossover and maximal preservation crossover 
                    is that in the position crossover the randomly selected elements are not necessarily in a sequence.
                </p>
                <p>This is an example of how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Crossover/position_crossover.png" alt="Position Crossover (PX)"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 40; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=40" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
def sorted_random_numbers(point_nums, interval):  <br>
    ...
    <br>
    <br>
"Position crossover"  <br>
def position_crossover(parent_one, parent_two, pc):  <br>
    """  <br>
    This function takes two parents, and performs Position crossover on them.  <br>
    In this method, we assumong the length of chromosomes are more than 5.  <br>
    The main difference of this crossover with Maximal-preservation crossover, is that the points   <br>
    that are being transfered directly to children, are not necessarily consequent.     <br>
    ...
    <br>
    parent_one = Chromosome(genes= np.array([1, 2, 3, 4, 5, 6, 7, 8]),id_=0,fitness = 125.2)  # parents  <br>
    parent_two = Chromosome(genes= np.array([2, 4, 6, 8, 7, 5, 3, 1]),id_=1,fitness = 125.2)  <br>
    <br>
    print("\nParents")  <br>
    print("=================================================")  <br>
    ...
    <br>
    <br>
    seen_list1 = [] #Elements that are transfered to child_one  <br>
    seen_list2 = [] #Elements that are transfered to child_two  <br>
    <br>
    if np.random.rand() < pc:  # if pc is greater than random number  <br>
        #********************************************************* Filling Child_one  <br>
        point_nums = np.random.randint(2,chrom_length-2) #number of points  <br>
        points = sorted_random_numbers(point_nums, chrom_length) #for choosing several points  <br>
        <br>
        for i in range(chrom_length):  <br>
            if i in points:  <br>
                child_one.genes[i] = parent_one.genes[i]  <br>
                seen_list1.append(parent_one.genes[i])  <br>
                <br>
        points = [] #finding empty spots in child_one  <br>
        for i in range(chrom_length):  <br>
            if child_one.genes[i] == -1:  <br>
                points.append(i)  <br>
        k = 0  <br>
        for i in range(chrom_length):  <br>
            if parent_two.genes[i] not in seen_list1:  <br>
                child_one.genes[points[k]] = parent_two.genes[i]  <br>
                k+=1  <br>
        #********************************************************* Filling Child_two  <br>
        point_nums = np.random.randint(2,chrom_length-2) #number of points  <br>
        points = sorted_random_numbers(point_nums, chrom_length) #for choosing several points  <br>
        <br>
        for i in range(chrom_length):  <br>
            if i in points:  <br>
                ...
                seen_list2.append(parent_two.genes[i])  <br>
                <br>
        points = [] #finding empty spots in child_two  <br>
        for i in range(chrom_length):  <br>
            if child_two.genes[i] == -1:  <br>
                points.append(i)  <br>
        k = 0  <br>
        for i in range(chrom_length):  <br>
            if parent_one.genes[i] not in seen_list2:  <br>
                child_two.genes[points[k]] = parent_one.genes[i]  <br>
                k+=1  <br>
                <br>
...
CROSS = position_crossover(parent1, parent2, 1)  <br>
<br>
