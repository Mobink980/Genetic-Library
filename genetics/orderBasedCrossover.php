...
<ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Order Based Crossover (OBX)</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="modifiedPartiallyMappedCrossover.php" class="btn"> Previous</a>
        <a id="btnNext" href="positionBasedCrossover.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Order Based Crossover (OBX)</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In order based crossover, we choose three random genes from parent two and we 
                    find their place in Parent one. we keep these places in child one empty and
                    fill every other position in child one by transferring the genes from parent 
                    one directly to them. then we loop over parent two and transfer to child one
                    the genes that are not already transferred to it. for child two we already 
                    saved the Chosen places in Parent one, so we keep the same places empty in 
                    child two and fill every other position in child two, by transferring the genes
                    from parent two directly to them. Then we loop over parent one and transfer to 
                    child two the genes that are not already transferred to it.
                </p>
           
                <p>This is an example for how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Crossover/order_based_crossover.png" alt="Order Based Crossover (OBX)"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 36; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=36" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
def sorted_random_numbers(point_nums, interval):   <br>
    ...
    <br>
"Ordered based crossover"   <br>
def ordered_based_crossover(parent_one, parent_two, pc):   <br>
    """   <br>
    This function takes two parents, and performs Ordered based crossover on them.     <br>
    ...
    if np.random.rand() < pc:  # if pc is greater than random number   <br>
        ...
        for i in range(len(points)):   <br>
            not_seen_list1.append(parent_one.genes[points[i]])   <br>
            not_seen_list2.append(parent_two.genes[points[i]])   <br>
            <br>
        for i in range(chrom_length): #transfering genes of parents to childs except chosen positions   <br>
            if i not in points:   <br>
                child_one.genes[i] = parent_one.genes[i]   <br>
                child_two.genes[i] = parent_two.genes[i]   <br>
                <br>
        k = 0   <br>
        for i in range(chrom_length):   <br>
            if parent_two.genes[i] in not_seen_list1: #elements in parent_two that are not in child_one   <br>
                child_one.genes[points[k]] = parent_two.genes[i]   <br>
                k+=1   <br>
        k = 0   <br>
        for i in range(chrom_length):   <br>
            if parent_one.genes[i] in not_seen_list2: #elements in parent_one that are not in child_two   <br>
...
CROSS = ordered_based_crossover(parent1, parent2, 1)   <br>
<br>
