...
<ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Alternate Edges Crossover (AEX)</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="edgeRecombinationCrossover.php" class="btn"> Previous</a>
        <a id="btnNext" href="heuristicUniformCrossover.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Alternate Edges Crossover (AEX)</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    to operate alternate edges crossover on two selected parents we first need 
                    to introduce a concept named arc. An arc is any pair of elements that are 
                    in a sequence. for instance, the arcs for the chromosome parent one is as follows:
                </p>
                <p>
                    2 ==> 3 (An image is required.)
                </p>
                <p>Now we discuss how alternate edges crossover works with an example.</p>



<img class="image-view" src="Genetics/Genetic pictures/Crossover/alternate_edges_crossover.png" alt="Alternate Edges Crossover (AEX)"/> 

<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 44; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=44" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
...
class Chromosome():   <br>
    ...
    def get_chrom_length(self):    <br>
        ...
        <br>
def duplicate_list(myList):   <br>
    """   <br>
    Returns True if there are no duplicate elements in the list   <br>
    """    <br>
    if len(myList) == len(set(myList)):   <br>
        return False   <br>
        <br>
    return True   <br>

    <br>   <br>
"Alternate edges crossover"   <br>
def alternate_edges_crossover(parent_one, parent_two, pc):    <br>
    """   <br>
    This function takes two parents, and performs Alternate edges crossover on them.    <br>
    ...
    <br>
        #********************************************************************** For the first child   <br>
        target_arcs = []   <br>
        seen_list = []   <br>
        pos = 0 #position of the next element   <br>
        count = 0   <br>
        alternate = 0   <br>
        while True:   <br>
            if count == chrom_length:   <br>
                break   <br>
            if alternate%2 == 0:   <br>
                target_arcs.append(parent_one.genes[pos])   <br>
                seen_list.append(parent_one.genes[pos])   <br>
                if pos+1 != chrom_length:   <br>
                    pos+=1     <br>
                count+=1   <br>
                pos = parent_two.genes.tolist().index(parent_one.genes[pos])    <br>
                alternate+=1   <br>
            else:   <br>
                target_arcs.append(parent_two.genes[pos])   <br>
                seen_list.append(parent_two.genes[pos])   <br>
                if pos+1 != chrom_length:   <br>
                    pos+=1     <br>
                count+=1   <br>
                pos = parent_one.genes.tolist().index(parent_two.genes[pos])   <br>
                alternate+=1   <br>
                <br>   <br>

            if count > 1:   <br>
                n = 0   <br>
                for k in range(len(seen_list) - 1):   <br>
                    n+=1   <br>
                    if target_arcs[-1] == seen_list[k]:    <br>
                        break   <br>
                        <br>
            not_seen_list = [] #Elements that are not already transfered to child   <br>
            if count > 1 and n != len(seen_list) - 1: #if a cycle is found   <br>
                target_arcs.pop()   <br>
                for j in range(chrom_length):   <br>
                    if parent_one.genes[j] not in seen_list:   <br>
                        not_seen_list.append(parent_one.genes[j])   <br>
                if not_seen_list == []:   <br>
                    break   <br>
                    <br>
                if alternate%2 != 0: #it means that we reached a cycle in parent_two   <br>
                    element = not_seen_list[np.random.randint(0, len(not_seen_list))]   <br>
                    target_arcs.append(element)   <br>
                    seen_list.append(element)   <br>
                    pos = parent_one.genes.tolist().index(element)   <br>
                    if pos+1 != chrom_length:   <br>
                        pos+=1                     <br>
                    pos = parent_two.genes.tolist().index(parent_one.genes[pos])    <br>
                    <br>
                    <br>
                else: #we reached a cycle in parent_one   <br>
                    element = not_seen_list[np.random.randint(0, len(not_seen_list))]   <br>
                    target_arcs.append(element)    <br>
                    seen_list.append(element)   <br>
                    pos = parent_two.genes.tolist().index(element)    <br>
                    if pos+1 != chrom_length:   <br>
                        pos+=1     <br>
                    pos = parent_one.genes.tolist().index(parent_two.genes[pos])    <br>
                    <br>
                    <br>
        #$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ For assurance that child_one is an authentic answer   <br>
        duplicated_elements = [item for item, count in collections.Counter(target_arcs).items() if count > 1]   <br>
        if duplicate_list(target_arcs):   <br>
            not_seen_list = []   <br>
            for i in range(chrom_length): #filling the not_seen_list    <br>
                if parent_one.genes[i] not in target_arcs:   <br>
                    not_seen_list.append(parent_one.genes[i])   <br>
            for i in range(len(duplicated_elements)): #replacing the duplicated elements with the elements that are not seen   <br>
                location = len(target_arcs) - 1 - target_arcs[::-1].index(duplicated_elements[i]) #last duplicated element   <br>
                target_arcs[location] = not_seen_list.pop()   <br>
        #$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$   <br>
        child_one.genes = np.array(target_arcs)   <br>
        <br>
        # target_arcs = list(dict.fromkeys(target_arcs))   <br>
        # seen_list = list(dict.fromkeys(seen_list))   <br>
        #************************************************************************************ For the second child   <br>
        target_arcs = []   <br>
        seen_list = []   <br>
        pos = 0 #position of the next element   <br>
        count = 0   <br>
        ...
        while True:   <br>
            ...
            if count > 1 and n != len(seen_list) - 1: #if a cycle is found   <br>
                ...
                if alternate%2 != 0: #it means that we reached a cycle in parent_two   <br>
                    ...
                    pos = parent_one.genes.tolist().index(parent_two.genes[pos])    <br>
                    <br>
        #$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ For assurance that child_two is an authentic answer   <br>
        duplicated_elements = [item for item, count in collections.Counter(target_arcs).items() if count > 1]   <br>
        if duplicate_list(target_arcs):   <br>
            not_seen_list = []   <br>
            for i in range(chrom_length): #filling the not_seen_list    <br>
            ...
            for i in range(len(duplicated_elements)): #replacing the duplicated elements with the elements that are not seen   <br>
                ...
                target_arcs[location] = not_seen_list.pop()   <br>
        #$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$   <br>
        child_two.genes = np.array(target_arcs)   <br>
        <br>   <br>
...
CROSS = alternate_edges_crossover(parent1, parent2, 1)   <br>
<br>
