...
<ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Position Based Crossover (POS)</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="orderBasedCrossover.php" class="btn"> Previous</a>
        <a id="btnNext" href="votingRecombinationCrossover.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Position Based Crossover (POS)</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In position-based crossover, we choose several random genes 
                    from parent two and directly transfer them to child one. 
                    Then we fill the empty positions in child one, by looping over the 
                    parent one and transferring all the genes that are not already transferred 
                    to child one from parent one to child one. for child two we transfer the 
                    genes in the positions that earlier were randomly chosen, from parent one 
                    directly to child two. Then we loop over the parent two and transfer all 
                    the genes that are not already transferred to child two from parent two to child two.
                </p>

                <p>This is an example for how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Crossover/position_based_crossover.png" alt="Position Based Crossover (POS)"/>
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 37; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=37" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?> 
<div class="code">
...
def position_based_crossover(parent_one, parent_two, pc):   <br>
    """   <br>
    This function takes two parents, and performs Position based crossover on them.     <br>
    ...
    if np.random.rand() < pc:  # if pc is greater than random number   <br>
        "points in the begining, is the positions that are to be transfered from parent_two to child_one."   <br>
        "Also the positions that are to be transfered from parent_one to child_two."   <br>
        points = sorted_random_numbers(3,chrom_length)     <br>
        # points[0] = 1   <br>
        # points[1] = 2   <br>
        # points[2] = 5         <br>
        for i in range(len(points)):   <br>
            seen_list1.append(parent_two.genes[points[i]])   <br>
            seen_list2.append(parent_one.genes[points[i]])   <br>
            <br>
        for i in range(chrom_length): #transfering genes of parents to childs in the chosen positions   <br>
            if i in points:   <br>
                child_one.genes[i] = parent_two.genes[i]   <br>
                child_two.genes[i] = parent_one.genes[i]   <br>
                <br>
        points = [] #positions in the childs that are not already filled   <br>
        ...
        k = 0   <br>
        for i in range(chrom_length):   <br>
            if parent_one.genes[i] not in seen_list1: #elements in parent_one that are not in child_one   <br>
                child_one.genes[points[k]] = parent_one.genes[i]   <br>
                k+=1   <br>
                <br>
        k = 0   <br>
        for i in range(chrom_length):   <br>
            if parent_two.genes[i] not in seen_list2: #elements in parent_two that are not in child_two   <br>
                child_two.genes[points[k]] = parent_two.genes[i]   <br>
                k+=1   <br>
                <br>
...
CROSS = position_based_crossover(parent1, parent2, 1)   <br>
<br>
