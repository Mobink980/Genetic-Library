...
<div id="leftSideBar" style="margin-top: 90px;">
            <ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Center Inversion Mutation (CIM)</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="inorderMutation.php" class="btn"> Previous</a>
        <a id="btnNext" href="throasMutation.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Center Inversion Mutation (CIM)</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In center inversion mutation, we divide the chromosome into two parts;
                    then we reverse each part and transfer the result to the offspring.
                </p>
                <p>This is an example for how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Mutation/center_inversion_mutation.png" alt="Center Inversion Mutation (CIM)"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 59; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=59" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
def sorted_random_numbers(point_nums, interval): <br>
    ...
    <br>
    <br>
"Center inversion mutation" <br>
def center_inversion_mutation(parent_one, pm): <br>
    """ <br>
    This function takes one chromosome, and performs Center inversion mutation on it. <br> 
    Length of the chromosome in this mutation should be more than 4. <br>
    ...
    if np.random.rand() < pm:  # if pm is greater than random number <br>
        reverse_list1 = [] <br>
        reverse_list2 = [] <br>
        point = np.random.randint(2,chrom_length-1) #break point <br>
        print("point", point)     <br>
        child_one.genes = parent_one.genes <br>
        for i in range(chrom_length): <br>
            if i < point: <br>
                reverse_list1.append(parent_one.genes[i]) <br>
            else: <br>
                reverse_list2.append(parent_one.genes[i]) <br>
        reverse_list1.reverse() <br>
        reverse_list2.reverse() <br>
        print("reverse_list1:", reverse_list1) <br>
        print("reverse_list2:", reverse_list2) <br>
        <br>
        for i in range(point): #Filling the first half <br>
            child_one.genes[i] = reverse_list1[i] <br>
        k = 0 <br>
        for i in range(point, chrom_length): #Filling the first half <br>
            child_one.genes[i] = reverse_list2[k] <br>
            k+=1 <br>
            <br>
...
MUTATION = center_inversion_mutation(parent1, 1) <br>
<br>
