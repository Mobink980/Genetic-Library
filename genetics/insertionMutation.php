...
<div id="leftSideBar" style="margin-top: 90px;">
            <ul>
                ...
                <li id="leftSideBarSelected"><a href="#">Insertion Mutation</a></li>
    ...
    <div class="topButtons">
        <a id="btnPrev" href="displacementMutation.php" class="btn"> Previous</a>
        <a id="btnNext" href="displacedInversionMutation.php" class="btn">Next</a>
    </div>
        <div class="explain">

        <h1>Insertion Mutation</h1>
                <hr />
                <!--explanation needs fixing-->
                <p>
                    In insertion mutation, we select a random gene in the chromosome and 
                    we paste that gene in another position, without changing the sequence 
                    of other elements. The main difference between insertion mutation and
                    displacement mutation is that in insertion mutation, we change the 
                    position of only one element.
                </p>
                <p>This is an example for how it works.</p>

<img class="image-view" src="Genetics/Genetic pictures/Mutation/insertion_mutation.png" alt="Insertion Mutation"/> 
<?php
    
    include 'includes/dbh.inc.php';
    $page_id = 63; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=63" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
def sorted_random_numbers(point_nums, interval):  <br>
    ...
    <br>
    <br>
"Insertion mutation"  <br>
def insertion_mutation(parent_one, pm):  <br>
    """  <br>
    This function takes one chromosome, and performs Insertion mutation on it.   <br>
    ...
    if np.random.rand() < pm:  # if pm is greater than random number  <br>
        point = np.random.randint(0, chrom_length) #picking one gene  <br>
        # print(point)      <br>
        duplicate = parent_one.genes.tolist().copy()           <br>
        duplicate.remove(parent_one.genes[point])   <br>
        # print(duplicate)  <br>
        <br>
        lists = []  <br>
        for i in range(len(duplicate) + 1):  <br>
            a = []  <br>
            lists.append(a)  <br>
        position = np.random.randint(0, len(duplicate) + 1) #the position of the paste  <br>
        # print(position)  <br>
        lists[position].append(parent_one.genes[point])  <br>
        k = 0  <br>
        for i in range(len(lists)):  <br>
            if not lists[i]: #if lists[i] is empty  <br>
                lists[i].append(duplicate[k])  <br>
                k+=1  <br>
        # print(lists)  <br>
        flat_list = [item for sublist in lists for item in sublist]  <br>
        child_one.genes = np.array(flat_list)  <br>
        <br>
...
MUTATION = insertion_mutation(parent1, 1)  <br>
<br>
