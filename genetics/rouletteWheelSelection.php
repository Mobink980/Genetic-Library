...
<div id="leftSideBar" style="margin-top: 90px;">
        <ul>
            ...
            <li id="leftSideBarSelected"><a href="#">Roulette Wheel  Selection</a></li>
    ...
    <div class="topButtons">
        ...
        <h1>Roulette Wheel  Selection</h1>
                <hr />
                <p>
                    In Roulette wheel Selection, Each individual has the probability of fitness(x(i))/total_fitnesses, to be selected. (the selection is biased towards individuals with better fitnesses)
                    Roulette wheel selection with stochastic acceptance:
                </p>
                <p>
                    The Roulette wheel selection via stochastic acceptance algorithm does 
                    not use searching but is based on a stochastic acceptance of a randomly selected individual.
                </p>
                <p>
                ...
                <p>prob'[i] = (fitness(chromosome[i])/(pop_size*maximum_fitness)) * (1 + q + q^2 + q^3 + ...)</p>
                <p>
                    The geometrical series (2) is convergent (0 &lt q &lt 1), which gives that prob'[i]=prob[i] Based on 
                    Maclaurin series we conclude: 
                </p>
                <p>(1 + q + q^2 + q^3 + ...)=1/(1 - q), which q is the rejection probability.</p>


      <?php
    
    include 'includes/dbh.inc.php';
    $page_id = 8; //get the page_id from the url
    $sql = "SELECT * FROM search WHERE page_id= '$page_id'";           
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    $number = $row['page_like'];
    echo '<a href="increaseLike.php?pageid=8" class="btn btn-success" id ="like"><span class="glyphicon glyphicon-thumbs-up"></span> <span class="badge" id = "num">'.$number.'</span></a>'; 
      ?>
<div class="code">
...
def fitness_function(chrom): #TODO needs to be more generalize and useful <br>
    ...
    return fitness <br>
    <br>
def roulette_wheel_selection(selection_size, pop): <br>
    """ <br>
    In Roulette wheel Selection, Each individual has the probability of fitness(x(i))/total_fitnesses,  <br>
    ...
    # print(probs)  <br>
    # print(wheel) <br>
    for i in range(selection_size): #picking the individuals from the pool <br>
        chosen = 0 #the index of the chosen chromosome <br>
        random_number = np.random.uniform(low=0.0, high=1.0)  <br>
        for j in range(len(wheel)): <br>
            if random_number <= wheel[j]: <br>
                chosen = j <br>
                break <br>
                <br>
        # print(random_number) <br>
        # print(chosen) <br>
...
for i in range(len(POP)): #printing the population <br>
    ...
    <br>
SELECTION = roulette_wheel_selection(5, POP) <br>
<br>
